...
def TTZZZZE(expr,ca,cb,cc,cd,RA,RB,RC,RD,IOP=0):
    ...
    valueslist=[ca,cb,cc,cd]+RA+RB+RC+RD
                        ...
                        for J in range(N):
                            for K in range(N):
                                for L in range(N):
                                    ...
                                    if ID!=KD or JD!=LD:
                                        #V=V+sympy.N(T0000(ca,cb,cc,cd,RA,RB,RC,RD))*DA[I]*DB[J]*DC[K]*DD[L]/ca/cb/cc/cd/8
                                        V=V+TTZZZZE(fzzzz,ca,cb,cc,cd,RA,RB,RC,RD)*DA[I]*DB[J]*DC[K]*DD[L]/ca/cb/cc/cd/8
                                    #if [ID,JD,KD,LD]==[0,3,1,2]:
                                    #print(V)
                                    else:
                                        # Provisionally compute [A,B|A+T,B+T] with the symbol T.
                                        V=V+TTZZZZE(fzzzzABAB,ca,cb,cc,cd,RA,RB,RC,RD)*DA[I]*DB[J]*DC[K]*DD[L]/ca/cb/cc/cd/8
...
VAB_C=[[[0 for _ in range(4)] for _  in range(4)] for _ in range(4)]
for ID in range(4):
    for JD in range(4):
        for KD in range(4):
                RA=[AX,AY,AZ]=atoms[ID]
...
SAB=[[0 for _ in range(4)] for _  in range(4)]              
for ID in range(4):
    for JD in range(4):
        RA=[AX,AY,AZ]=atoms[ID]
        RB=[BX,BY,BZ]=atoms[JD]
...
KAB=[[0 for _ in range(4)] for _  in range(4)]              
for ID in range(4):
    for JD in range(4):
        RA=[AX,AY,AZ]=atoms[ID]
        RB=[BX,BY,BZ]=atoms[JD]
...
#
    ...
    P2=[[0 for _ in range(4)] for _ in range(4)]
    G=[[0 for _ in range(4)] for _ in range(4)]
    G2=[[0 for _ in range(4)] for _ in range(4)]
    F=[[0 for _ in range(4)] for _ in range(4)]
    F2=[[0 for _ in range(4)] for _ in range(4)]
...
SPART=series(GetNormS([x,y,u,w],SAB)-1,R,1.7).removeO().expand()
SPART2=series(GetNormS([p,q,r,s],SAB)-1,R,1.7).removeO().expand()
SPART3=series(vSw([p,q,r,s],SAB,[x,y,u,w]),R,1.7).removeO().expand()
...
OBJ=OBJ0.subs([(p,0),(q,0),(r,0),(s,0),(f,0)])
