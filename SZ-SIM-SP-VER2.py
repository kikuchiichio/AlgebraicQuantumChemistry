...
IVABCZZ=IVABC.diff(AZ).diff(BZ)/z1/z2/4 #(PZ;A|1/rC|PZ;B)
...
fT=fssss.subs(z1,1.).subs(z2,1.).subs(z3,1.).subs(z4,1.4).subs(AX,0).subs(AY,0).subs(AZ,0.).subs(BX,0).subs(BY,0).subs(BZ,0).\
subs(CX,0).subs(CY,0).subs(CZ,0).subs(DX,0).subs(DY,0).subs(DZ,T)
sympy.series(fT,T)
...
def GetTwoEIntegralABAB(key1,key2,key3,key4):
    #
    #  This function computes [(z1,A)(z1,B)|(z1,A)(z1B)]
    #  Hence it is inappropriate to compute [(z1,A)(z2,B)|(z3,A)(z4B)], which shall ocuur ST-3G.
    # 
    #
    AX,AY,AZ,BX,BY,BZ,CX,CY,CZ,DX,DY,DZ=symbols("AX AY AZ BX BY BZ CX CY CZ DX DY DZ")
    z1,z2,z3,z4=symbols("z1 z2 z3 z4")
...
def GetTwoEIntegralAAAA(key1,key2,key3,key4):
    #  To compute [(z1,A)(z1,A)|(z1,A)(z1A)]
    #  Hence it is inappropriate to compute [(z1,A)(z2,A)|(z3,A)(z4,A)], which shall ocuur ST-3G.
    # 
...
def GetTwoEIntegralABABz1234(key1,key2,key3,key4):
    AX,AY,AZ,BX,BY,BZ,CX,CY,CZ,DX,DY,DZ=symbols("AX AY AZ BX BY BZ CX CY CZ DX DY DZ")
    z1,z2,z3,z4=symbols("z1 z2 z3 z4")
    T=symbols("T",positive=True)
    keys1234=key1+key2+key3+key4
    if IntegralslistABAB.get(keys1234)!=None:
        return IntegralslistABAB.get(keys1234)
    f=GetTwoEIntegral(key1,key2,key3,key4)
    fABAB=f.subs([(CX,AX),(CY,AY),(CZ,AZ),(DX,BX),(DY,BY),(DZ,BZ+T)])
    IntegralslistABAB[keys1234]=fABAB
    return fABAB

def GetTwoEIntegralAAAAz1234(key1,key2,key3,key4):
    AX,AY,AZ,BX,BY,BZ,CX,CY,CZ,DX,DY,DZ=symbols("AX AY AZ BX BY BZ CX CY CZ DX DY DZ")
    z1,z2,z3,z4=symbols("z1 z2 z3 z4")
    T=symbols("T",positive=True)
    keys1234=key1+key2+key3+key4
    if IntegralslistAAAA.get(keys1234)!=None:
        return IntegralslistAAAA.get(keys1234)
    fABAB=GetTwoEIntegralABABz1234(key1,key2,key3,key4)
    fAAAA=fABAB.subs([(BX,AX),(BY,AY),(BZ,AZ+T)])
    IntegralslistAAAA[keys1234]=fAAAA
    return fAAAA
            ...
            for KD in range(NORBITALS):
                for LD in range(NORBITALS):
                    ...
                    if ID==KD and JD==LD and ID!=JD:
                        formula=GetTwoEIntegralABABz1234(key1,key2,key3,key4)       
                    if ID==KD and JD==LD and ID==JD:
                        formula=GetTwoEIntegralAAAAz1234(key1,key2,key3,key4)   
                    ...
                    if TTlist.get(indexijkl(ID,JD,KD,LD))==None:
                        ...
                        N=3
                        N=3
                        #print("WARN! We set N=1, because this run is to ascertain the well-composedness of the algorithm")
                        #print("We must set N=3 for STO-3G.")
                        V=0
                        VW=0
                        for I in range(N):            
                            for J in range(N):
                                for K in range(N):
                                    for L in range(N):
                                        ...
                                        if ID!=KD or JD!=LD:
                                            #print("1",formula)
                                            V=V+sympy.N(T0000(ca,cb,cc,cd,RA,RB,RC,RD))*DA[I]*DB[J]*DC[K]*DD[L]
                                            #VW=VW+TTZZZZE(formula,ca,cb,cc,cd,RA,RB,RC,RD)*DA[I]*DB[J]*DC[K]*DD[L]
                                        ...
                                        #print(V)
                                            ...
                                            #WV=VW+TTZZZZE(formula,ca,cb,cc,cd,RA,RB,RC,RD)*DA[I]*DB[J]*DC[K]*DD[L]
                        #if [ID,JD,KD,LD]==[0,3,1,2]:
                            #print(sympy.N(T0000(ca,cb,cc,cd,atoms[ID],atoms[JD],atoms[KD],atoms[LD])))
    ...
    for ID in range(NORBITALS):
        for JD in range(NORBITALS):
            for KD in range(NORBITALS):
                    ...
                    #print(key1,key2)
                    formula=0
                    formula=GetVintegral(key1,key2)
                    #print(formula==VABC)
                    ...
                    for I in range(N):            
                        for J in range(N):
                                ca=CA[I]
                                cb=CB[J]
                                #expr=TTZZZZE(formula,ca,cb,0,0,RA,RB,RC,[0,0,0])
                                
                                #if ID==JD==KD:
                                #    VAAA=V000000(ca,cb,[AX,AY,AZ],[AX,AY,AZ],[AX,AY,AZ+T])
                                #    expr=VAAA
                                #expr=expr.expand()
                                #print(V000000(ca,cb,RA,RB,RC),expr,formula)
                                #try:
                                #    print(I,J,sympy.N(V000000(ca,cb,RA,RB,RC0)),sympy.N(sympy.series(expr,T,0).removeO().subs(T,0)))
                                #except:
                                #    print(sympy.N(V000000(ca,cb,RA,RB,RC0)),expr)
#                                      sympy.N(V000000(ca,cb,RA,RB,RC)-sympy.series(expr,T,0).removeO().subs(T,0)))
#                                print(V000000(ca,cb,RA,RB,RC),TTZZZZE(formula,ca,cb,0,0,RA,RB,RC,[0,0,0]))
                                #V=V+V000000(ca,cb,RA,RB,RC)*DA[I]*DB[J]*(-ZC)
                                V+=V000000(ca,cb,RA,RB,RC)*DA[I]*DB[J]*(-ZC)
                    #if ID==JD==KD and ca==cb:
                    #    formula=GetVAAAintegral(key1,key2)
                    #    V=0
                    #    for I in range(N):            
                    #        for J in range(N):
                    #            ca=CA[I]
                    ...
                    #           expr=TTZZZZE(formula,ca,cb,0,0,RA,RB,RC,[0,0,0])
                    #            print(V000000(ca,cb,RA,RB,RC),expr,formula)
                    #            print("*",I,J,V000000(ca,cb,RA,RB,RC),sympy.series(expr,T,0).removeO().subs(T,0),V000000(ca,cb,RA,RB,RC)-sympy.series(expr,T,0).removeO().subs(T,0))
                    #            #V000000(a,b,RA,RB,RC)
                    #            #V=V+V000000(ca,cb,RA,RB,RC)*DA[I]*DB[J]*(-ZC)
                    #            V+=TTZZZZE(formula,ca,cb,0,0,RA,RB,RC,[0,0,0])*DA[I]*DB[J]*(-ZC)
    ...
    for ID in range(NORBITALS):
        for JD in range(NORBITALS):
            ...
            key1=ORBITKEYS[ID]
            key2=ORBITKEYS[JD]
            formula=GetSintegral(key1,key2)
            N=3
            for i in range(N):
                #print(i,EXPON[N-1][i],COEF[N-1][i],ZETA1,ZETA2)
                #CA[i]=EXPON[N-1][i]*ZETAS[ID]**2
    ...
    for ID in range(NORBITALS):
        for JD in range(NORBITALS):
            ...
            key1=ORBITKEYS[ID]
            key2=ORBITKEYS[JD]
            formula=GetKintegral(key1,key2)
            N=3
            for i in range(N):
                #print(i,EXPON[N-1][i],COEF[N-1][i],ZETA1,ZETA2)
                CA[i]=EXPON[N-1][i]*ZETAS[ID]**2
...
series(GetVAAAintegral((0,0,0),(0,0,0)),T)
...
for i in range(2):
    for j in range(2):
        for k in range(2):
            print(sympy.N(sympy.series(VAB_C[i][j][k],T,0).removeO().subs(T,0)))
...
for i in range(2):
    for j in range(2):
        for k in range(2):
            print(sympy.N(series(VAB_C[i][j][k],T,0).removeO().subs(T,0)))
...
ENT=sympy.N(sympy.series(ENT.expand(),T,0).removeO().subs(T,0))
